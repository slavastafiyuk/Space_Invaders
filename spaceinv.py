...
from Trees.DecisionTrees import *
from Trees.BehaviourTree import *
import math
...
class SpaceInvaders:
    ...
    def __init__(self, screen):
        ...
        self.ship_shield_pic = pygame.transform.scale(pygame.image.load(fileobj, namehint).convert(),
        ...
        self.alien_boss1_pic = pygame.transform.scale(self.alien_boss1_pic,
                                                      np.array(self.alien_boss1_pic.get_size()) * 2 // 3)
        self.pic_colorkey(self.alien_boss1_pic, (36, 36, 36))
        self.alien_boss1_cannon_pos = np.array(
            [[self.alien_boss1_pic.get_size()[0] * 0.2 - 10, self.alien_boss1_pic.get_size()[1] - 5],
        ...
        self.alien_boss2_cannon_pos = np.array(
            [[self.alien_boss2_pic.get_size()[0] * 0.43 - 10, self.alien_boss2_pic.get_size()[1] - 25],
        ...
        self.bullet_alien1_pic = pygame.transform.flip(pygame.transform.scale(self.bullet_alien1_pic, (
                np.array(self.bullet_alien1_pic.get_size()) / 2.6).astype(np.int16)), 0, 1)
        ...
        self.bullet_alien2_pic = pygame.transform.flip(
            pygame.transform.scale(self.bullet_alien2_pic, np.array(self.bullet_alien2_pic.get_size()) // 2), 0, 1)
        ...
        self.bullet_ship1_pic = pygame.transform.scale(self.bullet_ship1_pic,
                                                       np.array(self.bullet_ship1_pic.get_size()) // 3)
        ...
        self.explosion1_pic = self.adjust_pic(pygame.image.load(fileobj, namehint).convert(), np.array([-2, 7]),
                                              np.array([554, 554]))  # adjust picture position and size
        ...
        self.stars = np.random.rand(self.nr_stars, 3) * np.array(
            [self.screen_size[0] - 2, self.screen_size[1] - 2, 1.0]) \
        ...
        self.ship = Ship(self.ship_pic, self.bullet_ship1_pic, self.ship_shield_pic, self.ship_sound_gun,
                         self.ship_sound_explosion,
    ...
    def return_alien1_bullet_pic(self):
        return self.bullet_alien1_pic

    def return_alien2_bullet_pic(self):
        return self.bullet_alien2_pic
    ...
    def new_game(self, time):
        ...
        self.ship.alien_bullets_list.clear()
        self.scores.clear()
    ...
    def level_ops(self, time):
        ...
        if len(self.aliens) == 0 and len(self.ufos) == 0 and not self.game_over:
            if (self.level_completed_time > 120 and len(self.explosions) == 0 and len(
                    self.scores) == 0 and self.show_score == self.game_score) or self.level == 0:
                self.level_completed_time = 0
                self.level += 1
                # self.ship.aliens_target.clear()
                ...
                pic = self.font_game_score.render('LEVEL ' + str(self.level) + ' COMPLETED', True, self.title_color,
    ...
    def bullet_ops(self, time):
        ...
        for bullet in self.alien_bullets:
            ...
            if self.ship.shield:
                ...
                if self.ship.mask.overlap(bullet.mask, mask_offset):
                    ...
                    self.explosions.append(
                        Explosion(self.explosion_ship_pic, (int(self.ship.position[0] + self.ship.size[0] / 2),
            ...
            if bullet.position[0] < -bullet.size[0] or bullet.position[0] > self.screen_size[0] \
                ...
                # self.ship.alien_bullets_list.remove(bullet)
            else:
                ...
                if ufo.may_be_shot and bullet.rect.colliderect(ufo.rect):
                    ...
                    if ufo.mask.overlap(bullet.mask, mask_offset):
                        ...
                        self.scores.append(Score(self, ufo.score, str(ufo.score), self.font_score,
                                                 (int(bullet.position[0] + bullet.size[0]),
                                                  int(bullet.position[1] - 20)),
                                                 self.score_color, self.background_color))
                        self.play_sound(ufo.sound_explosion)
                        self.explosions.append(
                            Explosion(self.explosion_ship_pic, (int(ufo.position[0] + ufo.size[0] / 2),
                            ...
                            if not alien.hit_area or bullet.rect.colliderect(alien.hit_area.move(alien.position)):
                                ...
                                if alien.hit_nr == 0:
                                    ...
                                    if alien.hit_total > 5:
                                        ...
                                        self.explosions.append(Explosion(self.explosion_ship_pic,
                                                                         (int(alien.position[0] + alien.size[0] / 2),
                                                                          int(alien.position[1] + alien.size[1] / 2)),
                                                                         self.explosion_ship_pic_grid,
                                                                         self.explosion_ship_freq))
                                        ...
                                        # normal alien
                                        self.explosions.append(
                                            Explosion(self.explosion1_pic, (int(alien.position[0] + alien.size[0] / 2),
                                                                            int(alien.position[1] + alien.size[1] / 2)),
                                                      self.explosion1_pic_grid, self.explosion1_freq))
                                    self.aliens.remove(alien)
                                else:
                                    # hit but not killed
                                    self.scores.append(Score(self, int(alien.score / alien.hit_total),
                                                             str(int(alien.score / alien.hit_total)), self.font_score,
                                                             (int(bullet.position[0] + bullet.size[0]),
                                                              int(bullet.position[1] - 20)),
                                                             self.score_color, self.background_color))
                                    self.explosions.append(
                                        Explosion(self.explosion1_pic, (int(bullet.position[0] + bullet.size[0] / 2),
    ...
    def alien_ops(self, time, prev_time):
        ...
        for alien in self.aliens:
            ...
            if (alien.rect.colliderect(self.ship.rect) or alien.position[1] >= self.ship.position[
                1]) and not self.game_over:
            ...
            if not self.game_over and not self.freeze_aliens and random.random() < self.fps / alien.shoot_freq:
                for i in range(np.shape(alien.cannon_pos)[0]):
                    self.alien_bullets.append(
                        Bullet(alien.bullet_pic, alien.position + alien.cannon_pos[i, :], np.array([0.0, 6.5]),
                               np.array([0.0, 0.0])))
                    # self.ship.alien_bullets_list.append(
                    #    Bullet(alien.bullet_pic, alien.position + alien.cannon_pos[i, :], np.array([0.0, 6.5]),
                    #           np.array([0.0, 0.0])))
    ...
    def ufo_ops_back(self, time, prev_time):
        ...
        if random.random() < self.level_ufo_probability / 50 and (
                self.game_mode != 'game' or (len(self.aliens) > 0 and not self.freeze_aliens)):
            # add a ufo
            speed = 0.35 + (self.level ** 0.5) * random.random() * 0.1
            from_side = random.randint(0, 1) * 2 - 1
            self.ufos.append(
                Ufo(self.alien_ufo_pic, self.bullet_ufo1_pic, self.alien_ufo_sound_explosion, 150, speed, from_side,
                    self.screen_size))
            ...
            if ufo.phase >= ufo.turning_point + 75 / 2 and last_phase < ufo.turning_point + 75 / 2:
                ...
                for i in range(-2, 3):
                    self.alien_bullets.append(Bullet(ufo.bullet_pic,
                                                     (ufo.position[0] + ufo.size[0] / 2 - ufo.bullet_size[0] / 2,
                                                      ufo.position[1] + ufo.size[1]),
                                                     np.array([i, (2 - abs(i)) * 0.3]), np.array([0.0, 0.2])))
            if ufo.phase > ufo.turning_point + 75 and (
                    np.min(ufo.position + ufo.size) < 0 or np.max(ufo.position - self.screen_size)) > 0:
    ...
    def info_ops(self):
        ...
        for powerup in self.ship.powerups:  # powerup is an array of [pid, life_time], powerup_data a list of (pic, life_time, desc, text_pic)
            ...
            self.screen.blit(self.powerup_data[pid][3],
                             (x + self.powerup_data[pid][0].get_size()[0] + 6, y + 4))  # add powerup description
            ...
            pygame.draw.rect(self.screen, color, (
                x + self.powerup_data[pid][0].get_size()[0] + self.powerup_desc_width + 12, y + 4, p_width, 12))
            y += 20
        ...
        if self.show_score != self.game_score:
            ...
            self.score_pic = self.font_game_score.render(str(self.show_score), True, self.score_color,
                                                         self.background_color)
            self.score_pic.set_colorkey(self.background_color)
        ...
        for i in range(1, lives):
            self.screen.blit(self.ship.pic_small, (
                self.level_pic_position[0] - 20 - int(1.2 * i * self.ship.pic_small_size[0]),
                self.level_pic_position[1]))
    ...
    def game_over_ops(self, time):
        ...
        elif time > self.game_over_time + 2 * self.switch_time_ms and len(
                self.ship.bullets) == 0 and self.show_score == self.game_score:
    ...
    def setup_level(self, time):
        ...
        self.level_pic = self.font_game_score.render('Level ' + str(self.level), True, self.score_color,
                                                     self.background_color)
        self.level_pic.set_colorkey(self.background_color)
        self.level_pic_position = (self.screen_size[0] - self.level_pic.get_size()[0] - 10, 10)
                ...
                if 90 + (y + 4) * alien_size[1] * lvl_max_data[10] < self.ship.position[1]:
                    for x in range(lvl_max_data[7]):
                        ...
                        if (y + x) % 2 == 0:
                            self.aliens.append(Alien(
                                lvl_data[0], lvl_data[1], lvl_data[2], lvl_data[3], lvl_data[4],
                                lvl_data[5] * hit_nr, hit_nr, pos, lvl_data[11], lvl_data[12], x_times, move_delay,
                                (y + x) * 30, 1, shoot_freq
                            ))
                        else:
                            self.aliens.append(Alien(
                                lvl2_data[0], lvl2_data[1], lvl2_data[2], lvl2_data[3], lvl2_data[4],
                                lvl2_data[5] * hit_nr, hit_nr, pos, lvl_data[11], lvl_data[12], x_times, move_delay,
                                (y + x) * 30, 1, shoot_freq
                            ))
    ...
    def setup_level_data(self):
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien1_pic.get_size()[0] // 2 - 1, self.alien1_pic.get_size()[1]]], dtype=np.float),
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien2_pic.get_size()[0] // 2 - 1, self.alien2_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            28, 1,
            8, 5, 1.6, 1.4, 20, 45,
            # 0, 0, 1.6, 1.4, 20, 45,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien3_pic.get_size()[0] // 2 - 1, self.alien3_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            30, 1,
            7, 6, 1.6, 1.4, 20, 45,
            # 0, 0, 1.6, 1.4, 20, 45,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien4_pic.get_size()[0] // 2 - 1, self.alien4_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            32, 1,
            7, 7, 1.6, 1.3, 20, 50,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien5_pic.get_size()[0] // 2 - 1, self.alien5_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            35, 1,
            8, 7, 1.5, 1.4, 22, 45,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien6_pic.get_size()[0] // 2 - 1, self.alien6_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            38, 1,
            8, 8, 1.5, 1.3, 24, 50,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien7_pic.get_size()[0] // 2 - 1, self.alien7_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            40, 1,
            8, 8, 1.5, 1.4, 25, 50,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien8_pic.get_size()[0] // 2 - 1, self.alien8_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            42, 1,
            8, 8, 1.5, 1.3, 25, 50,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien9_pic.get_size()[0] // 2 - 1, self.alien9_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            44, 1,
            8, 8, 1.5, 1.3, 25, 50,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien10_pic.get_size()[0] // 2 - 1, self.alien10_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            46, 1,
            9, 8, 1.5, 1.4, 25, 50,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien11_pic.get_size()[0] // 2 - 1, self.alien11_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            48, 1,
            9, 9, 1.5, 1.4, 25, 50,
            500, 32000,
        ...
        self.level_data.append((
            ...
            self.bullet_alien1_pic,
            np.array([[self.alien12_pic.get_size()[0] // 2 - 1, self.alien12_pic.get_size()[1]]], dtype=np.float),
            self.alien1_sound_explosion,
            50, 1,
            9, 9, 1.5, 1.5, 25, 50,
            500, 32000,
    ...
    def setup_powerups(self, pic, font, font_desc):
        ...
        desc = 'Double Fire'
        powerup_data.append(
            self.setup_powerups_pic(pic.copy(), old_color, (120, 20, 220), desc, desc[:1], 24000, font, font_desc))
        desc = 'Triple Fire'
        powerup_data.append(
            self.setup_powerups_pic(pic.copy(), old_color, (120, 20, 20), desc, desc[:1], 15000, font, font_desc))
        desc = 'Rapid Fire'
        powerup_data.append(
            self.setup_powerups_pic(pic.copy(), old_color, (20, 20, 120), desc, desc[:1], 40000, font, font_desc))
        desc = 'Auto Fire'
        powerup_data.append(
            self.setup_powerups_pic(pic.copy(), old_color, (220, 20, 120), desc, desc[:1], -1, font, font_desc))
        desc = 'Freeze Aliens'
        powerup_data.append(
            self.setup_powerups_pic(pic.copy(), old_color, (160, 220, 160), desc, desc[:1], 5000, font, font_desc))
        desc = 'Shield'
        powerup_data.append(
            self.setup_powerups_pic(pic.copy(), old_color, (160, 160, 220), desc, desc[:1], 9000, font, font_desc))
    ...
    def setup_powerups_pic(self, new_pic, old_color, new_color, desc, letter, life_time, font, font_desc):
        ...
        new_pic.blit(pic_letter, ((new_pic.get_size()[0] - pic_letter.get_size()[0]) // 2,
                                  (new_pic.get_size()[1] - pic_letter.get_size()[1]) // 2))
    ...
    def setup_titles(self, time):
        ...
        title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist, '- TOP PLAYERS -', title_pos[1],
                                         None, 1.5)
        ...
        title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist, 'PLAYER', title_pos[1],
                                         hs_pos_x[0], 0.0)
        title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist, 'SCORE', title_pos[1],
                                         hs_pos_x[1], 0.0)
        title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist, 'LVL', title_pos[1], hs_pos_x[2],
                                         1.5)
        ...
        if self.game_mode == 'new high score':
            title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist,
                                             'CONGRATULATIONS! NEW HIGH SCORE', title_pos[1], None, 1.2)
            title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist,
                                             'ENTER YOUR NAME AND PRESS RETURN', title_pos[1], None, 1.2)
        else:
            title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist,
                                             'PRESS SPACE BAR FOR A NEW GAME', title_pos[1], None, 1.2)
            title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist,
                                             'ESC TO EXIT     I FOR INSTRUCTIONS', title_pos[1], None, 1.2)
        ...
        for i in range(0, self.highscore_nr):
            ...
            if hs[1] > 0:
                ...
                text = hs[0]
                if self.game_mode == 'new high score' and i == self.latest_hs_nr and len(
                        hs[0]) < self.highscore_name_length and time % 1000 > 500:
                    text = text + '_'
                title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist, text, title_pos[1],
                                                 hs_pos_x[0], 0.0)
                title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist, str(hs[1]), title_pos[1],
                                                 hs_pos_x[1], 0.0)
                title_pos[1] = self.add_text_row(self.screen_titles, self.font_highscorelist, str(hs[2]), title_pos[1],
                                                 hs_pos_x[2], 1.1)
                # test for overlap with end texts
                if title_pos[1] >= self.title_hs_pos_end[1]:
                    break

        subsurf = self.screen_titles.subsurface(
    ...
    def setup_credits(self):
        ...
        self.screen_credits.fill(self.background_color,
                                 (0, self.title_hs_pos[1], self.screen_size[0] - 1,
                                  self.title_hs_pos_end[1] - self.title_hs_pos[1] - 1))

        title_pos = self.title_hs_pos.copy()
        title_pos[1] = self.add_text_row(self.screen_credits, self.font_highscorelist, '- CREDITS -', title_pos[1],
                                         None, 3.0)
        title_pos[1] = self.add_text_row(self.screen_credits, self.font_highscorelist, 'LEAD PROGRAMMER', title_pos[1],
                                         None, 1.5)
        title_pos[1] = self.add_text_row(self.screen_credits, self.font_highscorelist, 'KALLE SAARIAHO', title_pos[1],
                                         None, 3.0)
        title_pos[1] = self.add_text_row(self.screen_credits, self.font_highscorelist, 'ASSISTANT PROGRAMMER',
                                         title_pos[1], None, 1.5)
        title_pos[1] = self.add_text_row(self.screen_credits, self.font_highscorelist, 'EELIS SAARIAHO', title_pos[1],
                                         None, 3.0)
        title_pos[1] = self.add_text_row(self.screen_credits, self.font_highscorelist, '(c) 2022', title_pos[1], None,
                                         1.1)

        subsurf = self.screen_credits.subsurface(
            (0, self.title_hs_pos[1], self.screen_size[0] - 1, self.title_hs_pos_end[1] - self.title_hs_pos[1] - 1))
    ...
    def setup_instructions(self):
        ...
        title_pos = self.title_hs_pos.copy()
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist, '- INSTRUCTIONS -',
                                         title_pos[1], None, 2.0)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'ALIEN INVASION IS IMMINENT.', title_pos[1], None, 2.0)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'YOU AND YOUR THREE SPACE FIGHTERS', title_pos[1], None, 1.1)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'ARE ALL THAT\'S LEFT TO STOP THEM.', title_pos[1], None, 1.5)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'WATCH OUT FOR UFOS AND THEIR BOMBS!', title_pos[1], None, 1.1)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'UFOS CAN ONLY BE SHOT IN THE MIDDLE', title_pos[1], None, 1.1)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'OF THEIR ATTACK PATTERN. THEY', title_pos[1], None, 1.1)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'MAY LEAVE POWER UPS IF SHOT DOWN.', title_pos[1], None, 1.5)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'USE LEFT AND RIGHT CURSOR KEYS TO', title_pos[1], None, 1.1)
        title_pos[1] = self.add_text_row(self.screen_instructions, self.font_highscorelist,
                                         'MOVE AND SPACE BAR TO FIRE.', title_pos[1], None, 1.1)

        subsurf = self.screen_instructions.subsurface(
            (0, self.title_hs_pos[1], self.screen_size[0] - 1, self.title_hs_pos_end[1] - self.title_hs_pos[1] - 1))
        ...
        self.title_rect = np.array([
            ...
            max(self.title_hs_rect[0] + self.title_hs_rect[2], self.title_ins_rect[0] + self.title_ins_rect[2],
                self.title_cr_rect[0] + self.title_cr_rect[2]) \
            - min(self.title_hs_rect[0], self.title_ins_rect[0], self.title_cr_rect[0]),
            max(self.title_hs_rect[1] + self.title_hs_rect[3], self.title_ins_rect[1] + self.title_ins_rect[3],
                self.title_cr_rect[1] + self.title_cr_rect[3]) \
            - min(self.title_hs_rect[1], self.title_ins_rect[1], self.title_cr_rect[1])
        ])

        # set z_pos of titles to control the rotation perspective
        self.title_z_pos = max(self.title_rect[3] * 3,
                               1.5 * (self.title_rect[3] / 2) / (self.screen_size[0] / self.title_rect[2] - 1))
    ...
    def rotate_titles(self, time):
        ...
        y_map_flat = ((y_mapr * np.ones((dst_rect[2]), dtype=np.int16)[:, None])[
            (x_map >= 0) & (x_map < self.title_rect[2])]).flatten()
    ...
    def move_stars(self, time, prev_time):
        ...
        self.stars += (time - prev_time) * self.star_move / np.hstack(
            (self.stars[:, 2:3], self.stars[:, 2:3], np.ones((self.nr_stars, 1))))
        ...
        self.stars[0:nr_half, :][(self.stars[0:nr_half, 2] > self.z_range[1])] = np.hstack((
            np.random.randint(0, 2, (
                np.shape(self.stars[0:nr_half, :][(self.stars[0:nr_half, 2] > self.z_range[1])])[0], 1)) * (
                    self.screen_size[0] - 2) - self.mid_screen[0],
            np.random.rand(np.shape(self.stars[0:nr_half, :][(self.stars[0:nr_half, 2] > self.z_range[1])])[0], 1) * (
                    self.screen_size[1] - 2) - self.mid_screen[1],
            np.random.rand(np.shape(self.stars[0:nr_half, :][(self.stars[0:nr_half, 2] > self.z_range[1])])[0], 1) * (
        ...
        self.stars[nr_half:, :][(self.stars[nr_half:, 2] > self.z_range[1])] = np.hstack((
            np.random.rand(np.shape(self.stars[nr_half:, :][(self.stars[nr_half:, 2] > self.z_range[1])])[0], 1) * (
                    self.screen_size[0] - 2) - self.mid_screen[0],
            np.random.randint(0, 2, (
                np.shape(self.stars[nr_half:, :][(self.stars[nr_half:, 2] > self.z_range[1])])[0], 1)) * (
                    self.screen_size[1] - 2) - self.mid_screen[1],
            np.random.rand(np.shape(self.stars[nr_half:, :][(self.stars[nr_half:, 2] > self.z_range[1])])[0], 1) * (
                    self.z_range[1] - self.z_range[0]) + self.z_range[0]
        ))
        # if Z too close OR X, Y out of bounds due to Z move, replace with a new random star at maximum Z
        self.stars[(self.stars[:, 2] < self.z_range[0]) | (abs(self.stars[:, 0] + 1) > self.mid_screen[0] - 1) | (
                abs(self.stars[:, 1] + 1) > self.mid_screen[1] - 1)] \
            = np.random.rand(np.shape(self.stars[(self.stars[:, 2] < self.z_range[0]) | (
                abs(self.stars[:, 0] + 1) > self.mid_screen[0] - 1) | (
                                                         abs(self.stars[:, 1] + 1) > self.mid_screen[1] - 1)])[0],
                             3) \
              * np.array([self.screen_size[0] - 2, self.screen_size[1] - 2, 0]) + np.array(
            [-self.mid_screen[0], -self.mid_screen[1], self.z_range[1]])
    ...
    def plot_stars(self):
        ...
        colors = (c_shades * ((1.0 - self.stars[:, 2:3] / (self.z_range[1] - self.z_range[0])) * 200 + 55)).astype(
            np.uint8)
    ...
    def pic_colorkey(self, pic, color):
        ...
        if color != (0, 0, 0) and color != (255, 255, 255):
            ...
            if color[0] + color[1] + color[2] <= 48 * 3:
                pic_array[:, :, 0:3][(pic_array[:, :, 0] <= color[0]) & (pic_array[:, :, 1] <= color[1]) & (
                        pic_array[:, :, 2] <= color[2])] = \
...
class Ship:
    ...
    def __init__(self, pic, bullet_pic, shield_pic, sound_gun, sound_explosion, screen_size, speed, shoot_freq, lives):
        ...
        self.ufos_target = []
        self.powerups_in_game = []
        self.powerups_position_list = []
        self.ufo_bullets_posicions = []
    ...
    def capsula_UFO(self):
        # Ponto central UFO
        ponto_central_x_UFO = self.space_ship_center_x()
        ponto_central_y_UFO = self.space_ship_center_y() - 70
        # Pontos para diagonal esquerda UFO
        diagonal_esquerda_x_UFO = self.position[0] + self.size[0] * 2 - 37  # 37
        diagonal_esquerda_y_UFO = self.space_ship_center_y() + 5
        # Pontos para diagonal direita UFO
        diagonal_direita_x_UFO = self.position[0] - self.size[0] + 37  # 37
    ...
    def move(self):
        ...
        self.left_square_ship = pygame.Rect(self.position[0] - self.size[0], self.position[1], self.size[0],
                                            self.size[1])
        self.center_square_ship = pygame.Rect(self.position[0] - 26, self.position[1] - self.size[1] - 30,
    ...
    def defineBehaviourTree(self):
        return Sequence(Selector(Sequence(Atomic(self.complement_poder_atacar_dis),
                                 ...
                                 Sequence(Atomic(self.powerups_exist),
                                          Atomic(self.get_powerup))
                                 )
                        )
    ...
    def sensor_line_esquerda(self):
        line_points = self.line_points(self.position[0] - 5, self.position[1] + 50, self.position[0] - 5,
                                       self.position[1] - 100)
        for p in range(len(self.alien_bullets_list)):
            for i in range(len(line_points)):
                if self.alien_bullets_list[p].rect.collidepoint(line_points[i]):
    ...
    def sensor_line_direita(self):
        line_points = self.line_points(self.position[0] + self.size[0] + 5, self.position[1] + 50,
                                       self.position[0] + self.size[0] + 5, self.position[1] - 100)
        for p in range(len(self.alien_bullets_list)):
            for i in range(len(line_points)):
                if self.alien_bullets_list[p].rect.collidepoint(line_points[i]):
                    return True
    ...
    def desviar(self):
        ...
        if self.ufo_bullet_intersect():
            for i in range(len(self.get_ufo_bullets_positions())):
                print("Bullet: " + str(self.get_ufo_bullets_positions()[i][1].position[0]) + " Mine: " + str(self.position[0]))
                if self.get_ufo_bullets_positions()[i][0] >= 0:
                    self.count_plus = + 1
                    self.list_desviar.insert(0, self.get_ufo_bullets_positions()[i][1].position[0]) ## insert 0 para balas a direita tendo em conta que o maximo de balas são 2
                elif self.get_ufo_bullets_positions()[i][0] < 0:
                    self.list_desviar.insert(1, self.get_ufo_bullets_positions()[i][1].position[0]) ## insert 1 para balas a esquerda tendo em conta que o maximo de balas são 2
                    self.count_nega = + 1
            ...
            if self.count_plus == self.count_nega:
                ...
                if self.position[0] > pos + 2:
                    self.move_sides_esquerda()
                elif self.position[0] < pos - 2:
                ...
                self.move_sides_esquerda()
        if self.lvl == 4:
            if 0 < self.get_bullets_boss_positions() < 60:
                self.move_sides_direita()
            elif 0 > self.get_bullets_boss_positions() > -60:
                self.move_sides_esquerda()
        else:
            if self.projetil_esquerda() and not self.projetil_direita():
                if self.status in (0, 1):
                    ...
                    self.shield_rect = pygame.Rect(self.position - (self.shield_size - self.size) // 2,
                                                   self.shield_size)
            elif self.projetil_direita() and not self.projetil_esquerda():
                if self.status in (0, 1):
                    self.position[0] += -1 * self.speed
                    if self.position[0] < 0:
                        self.position[0] = 0
    ...
    def ufo_bullet_intersect(self):
        for i in range(len(self.alien_bullets_list)):
            if self.alien_bullets_list[i].pic == self.ufo_bullet_pic and self.alien_bullets_list[i].rect.colliderect(
                    self.center_square_ship):
                return True
        return False
            ...
            if self.alien_bullets_list[i].rect.colliderect(self.center_square_ship):
                if self.alien_bullets_list[i].pic == self.ufo_bullet_pic:
                    ...
                    if len(self.ufo_bullets_posicions) < len(self.alien_bullets_list):
                        ...
                        self.ufo_bullets_posicions.sort(key=lambda row: (row[0][0], row[0][0]))
    ...
    def atacar_ufo_mais_proximo(self):  # VERIFICAR
        if self.get_ufo()[1].from_side == 1:  # RIGHT
            if self.position[0] + self.size[0] / 2 < self.get_ufo()[1].position[0] + self.get_ufo()[1].size[
                0] + 150 - 5:
                self.move_sides_direita()
            elif self.position[0] + self.size[0] / 2 > self.get_ufo()[1].position[0] + self.get_ufo()[1].size[
                0] + 150 + 5:
                self.move_sides_esquerda()
            else:
                self.disparar()
    ...
    def powerups_exist(self):
        if len(self.powerups_in_game) > 0 and self.poder_atacar_dis() and self.ufo_bullets_exist() == False and self.bullet_type != 2:
    ...
    def get_powerup_position(self):
        if len(self.powerups_position_list) > 0:
            self.powerups_position_list.pop(0)
    ...
    def get_powerup(self):
        if self.bullet_type == 2:
            pos = abs((self.position[1] + self.size[1] - self.get_powerup_position().position[1]) * math.tan(35))
            print(self.get_powerup_position().position[1])
            print(pos)
            if self.get_powerup_position().position[0] + self.get_powerup_position().size[0]/2 >= self.position[0] + self.size[0]/2:
                if self.position[0] + self.size[0] / 2 < self.get_powerup_position().position[0] + self.get_powerup_position().size[0]/2 - pos - 3:
                    self.move_sides_direita()
                    self.disparar()
                elif self.position[0] + self.size[0] / 2 < self.get_powerup_position().position[0] + self.get_powerup_position().size[0]/2 - pos + 3:
                    self.move_sides_esquerda()
                    self.disparar()
                else:
                    self.disparar()
            elif self.get_powerup_position().position[0] + self.get_powerup_position().size[0]/2 < self.position[0] + self.size[0]/2:
                if self.position[0] + self.size[0] / 2 < self.get_powerup_position().position[0] + self.get_powerup_position().size[0]/2 + pos - 3:
                    self.move_sides_direita()
                    self.disparar()
                elif self.position[0] + self.size[0] / 2 < self.get_powerup_position().position[0] + self.get_powerup_position().size[0]/2 + pos + 3:
            ...
            if self.position[0] + self.size[0] / 2 < self.get_powerup_position().position[0] + \
                    self.get_powerup_position().size[0] / 2 - 3:
                self.move_sides_direita()
                self.disparar()
            elif self.position[0] + self.size[0] / 2 > self.get_powerup_position().position[0] + \
                    self.get_powerup_position().size[0] / 2 + 3:
                self.move_sides_esquerda()
                self.disparar()
            else:
    ...
    def have_powerups(self):
        if self.alien_freeze or self.bullet_type == 2 or self.bullet_type == 3:
            return True
        else:
            return False
    ...
    def behavior_with_powerups(self):
        if self.obter_alien_mais_proximo().hit_area is None:
            #### FREEZE ###
            if self.alien_freeze and self.bullet_type == 1:
                if self.position[0] + self.size[0] / 2 < self.obter_alien_mais_proximo().position[0] + \
                    ...
                    self.disparar()
            ### FREEZE E DOUBLE FIRE ###
            elif self.alien_freeze and self.bullet_type == 2:
                if not self.left:
                    self.move_sides_esquerda()
                    self.disparar()
                    if self.position[0] >= 123:
                        self.left = True
                        self.right = False
                    elif self.position[0] <= self.obter_alien_mais_proximo().size[0] / 2 - 150:
                        self.left = True
                        self.right = False
                elif not self.right:
                    self.move_sides_direita()
                    self.disparar()
                    if self.position[0] <= 837:
                        self.left = False
                        self.right = True
                    if self.position[0] + self.size[0] >= self.obter_alien_mais_proximo().size[0] / 2 + 150:
                        self.left = False
                        self.right = True
            ### FREEZE E TRIPLE FIRE ###
            elif self.alien_freeze and self.bullet_type == 3:
                if self.position[0] + self.size[0] / 2 < self.obter_alien_mais_proximo().position[0] + \
                        self.obter_alien_mais_proximo().size[0] / 2 - 3:
                    self.move_sides_direita()
                    self.disparar()
                    ...
                    self.disparar()
            ### SEM FREEEZE COM DOUBLE FIRE ###
            elif not self.alien_freeze and self.bullet_type == 2:
                if not self.left:
                    self.move_sides_esquerda()
                    self.disparar()
                    if self.position[0] <= 123:
                    ...
                    self.move_sides_direita()
                    self.disparar()
                    if self.position[0] + self.size[0] >= 837:
                        self.left = False
                        self.right = True
            ### SEM FREEZE COM TRIPLE FIRE ###
            elif not self.alien_freeze and self.bullet_type == 3:
                if self.aliens_direction_right():
                    if self.position[0] < self.obter_alien_mais_proximo().position[0] + \
                            self.obter_alien_mais_proximo().size[
                                0] + self.calcular_afastamento() - 5:
                        self.move_sides_direita()
                        self.disparar()
                    elif self.position[0] > self.obter_alien_mais_proximo().position[0] + \
                            self.obter_alien_mais_proximo().size[0] + self.calcular_afastamento() + 5:
                        self.move_sides_esquerda()
                        self.disparar()
                    else:
                        self.disparar()
                elif self.aliens_direction_left():
                    if self.position[0] + self.size[0] < self.obter_alien_mais_proximo().position[
                        0] - self.calcular_afastamento() - 5:
                        self.move_sides_direita()
                        self.disparar()
                    elif self.position[0] + self.size[0] > self.obter_alien_mais_proximo().position[
                        0] - self.calcular_afastamento() + 5:
                        self.move_sides_esquerda()
                        self.disparar()
                    else:
                ...
                if not self.left:
                    ...
                    if self.position[0] >= 123:
                        ...
                        self.right = False
                            ...
                            self.obter_alien_mais_proximo().size[0] / 2 - 150:
                        self.left = True
                        self.right = False
                elif not self.right:
                    self.move_sides_direita()
            ...
            ### FREEZE E TRIPLE FIRE###
                ...
                if self.position[0] + self.size[0] / 2 < self.obter_alien_mais_proximo().position[0] + \
                    ...
                    self.disparar()
            ### SEM FREEZE COM DOUBLE FIRE###
            elif not self.alien_freeze and self.bullet_type == 2:
                if not self.left:
                    self.move_sides_esquerda()
                    self.disparar()
                    if self.position[0] <= 223:
                        self.left = True
                        self.right = False
                elif not self.right:
                    self.move_sides_direita()
                    self.disparar()
                    if self.position[0] + self.size[0] >= 737:
                        self.left = False
                        self.right = True
            ### SEM FREEZE COM TRIPLE FIRE###
            elif not self.alien_freeze and self.bullet_type == 3:
                if self.position[0] + self.size[0] / 2 < self.obter_alien_mais_proximo().position[0] + \
                        self.obter_alien_mais_proximo().size[0] / 2 - 10:
                    self.move_sides_direita()
                    self.disparar()
                if self.position[0] + self.size[0] / 2 > self.obter_alien_mais_proximo().position[0] + \
                        self.obter_alien_mais_proximo().size[0] / 2 + 10:
    ...
    def poder_atacar_dis_com_ufo_and_power(self):
        if self.shield:
            ...
            for i in range(len(self.alien_bullets_list)):
                if self.alien_bullets_list[i].rect.colliderect(
    ...
    def draw(self, screen):
        ...
        # Diagonal direita
        pygame.draw.line(screen, (255, 0, 0), (self.capsula()[0], self.capsula()[1]),
                         (self.capsula()[4], self.capsula()[5]))
        # Diagonal esquerda UFO
        # pygame.draw.line(screen, (255, 0, 0), (self.capsula_UFO()[0], self.capsula_UFO()[1]),
        #                 (self.capsula_UFO()[2], self.capsula_UFO()[3]))
        ...
        #                 pygame.Rect(self.position[0] - self.size[0], self.position[1] - 20, self.size[0], self.size[1] + 20))
        # Retangulo central
        # pygame.draw.rect(screen, (255, 0, 0), pygame.Rect(self.position[0] - 26, self.position[1] - self.size[1] - 30,
        #                                        self.size[0] + 50, self.size[1] + 70))
        # Linha direita
        # pygame.draw.line(screen, (255, 0, 0), (self.position[0] + self.size[0] + 5, self.position[1] + 50),
        #                 (self.position[0] + self.size[0] + 5, self.position[1] - 100))
        # Linha esquerda
        # pygame.draw.line(screen, (255, 0, 0), (self.position[0] - 5, self.position[1] + 50),
        #                 (self.position[0] - 5, self.position[1] - 100))
        # Linha desviar balas UFO
        # pygame.draw.line(screen, (255, 0, 0), (0 , 720), (960 , 720))
    ...
    def shoot(self, time):
        ...
        if time > self.last_shot_time + self.shoot_freq and (not self.space_down or self.auto_fire) and self.status in (
                0, 1):
            self.last_shot_time = time
            self.space_down = True
            # triple fire (3) combines single (1) and double (2)
            if self.bullet_type in (1, 3):
                self.bullets.append(Bullet(self.bullet_pic, self.position + np.array(
                    [(self.size[0] - self.bullet_size[0]) / 2, -self.bullet_size[1]]),
                                           np.array([0.0, -6.5]), np.array([0.0, 0.0])))
            if self.bullet_type in (2, 3):
                # Double Fire powerup
                self.bullets.append(Bullet(self.bullet_pic_left, self.position + np.array(
                    [self.size[0] / 2 - self.bullet_left_size[0], -self.bullet_left_size[1]]),
                                           np.array([np.cos((90.0 + self.double_fire_angle) * np.pi / 180) * 6.5,
                                                     np.sin((90.0 + self.double_fire_angle) * np.pi / 180) * -6.5]),
                                           np.array([0.0, 0.0])))
                self.bullets.append(Bullet(self.bullet_pic_right,
                                           self.position + np.array([self.size[0] / 2, -self.bullet_right_size[1]]),
                                           np.array([np.cos((90.0 - self.double_fire_angle) * np.pi / 180) * 6.5,
                                                     np.sin((90.0 - self.double_fire_angle) * np.pi / 180) * -6.5]),
                                           np.array([0.0, 0.0])))
    ...
    def move(self, time, level_time, screen_size):
        ...
        if self.x_times == 0:
            ...
            self.position = np.array([screen_size[0] * (0.5 + (s[0] + s[1]) * (screen_size[0] - self.size[0] * 1.1)
                                      ...
                                      screen_size[1] * (0.3 + (s[2] + s[3]) * (
                                              screen_size[1] / (0.5 / 0.3) - self.size[1] * 1.1)
    ...
    def draw(self, screen):
        ...
        if self.hit_total > 1:
            ...
            pygame.draw.rect(screen, (200, 0, 0),
                             (pos, (self.life_bar_size[0] * self.hit_nr // self.hit_total, self.life_bar_size[1])), 0)
...
class Ufo:
    ...
    def __init__(self, pic, bullet_pic, sound_explosion, score, speed, from_side, screen_size):
        ...
        self.size = (self.orig_size / self.z_pos_start).astype(
            np.int16)  # define start size in pixels (80) based on ufo pic width
        ...
        self.position_3D = np.array([
            ...
            -screen_size[1] / 2 * (random.random() * 0.2 + 0.05),
            # y start is between 0.25 and 0.45 times screen height
...
class Explosion:
    ...
    def draw(self, screen):
        rect = (int(self.phase[0] * self.grid_size[0]), int(self.phase[1] * self.grid_size[1]), int(self.grid_size[0]),
